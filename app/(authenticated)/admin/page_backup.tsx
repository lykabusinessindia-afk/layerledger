          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[840px] text-left">
              ...
              <tbody>
                {orders.map((order) => {
                  const hasSTL =
                    typeof order.file_url === "string" &&
                    order.file_url.startsWith("http");
                  ...
                  <tr key={order.id} className="border-b border-black/5 last:border-b-0 dark:border-white/10">
                    ...
                    <td className="px-4 py-3">
                      <button
                        className="mr-2 rounded bg-green-600 px-3 py-1 text-xs text-white"
                        onClick={() => handleDownloadDetails(order)}
                      >
                        Download Details
                      </button>
                      <button
                        className="mr-2 rounded bg-slate-600 px-3 py-1 text-xs text-white"
                        onClick={() => handleViewCustomization(order)}
                      >
                        View Customization
                      </button>
                      <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                        <span>{hasSTL ? "Yes" : "No"}</span>
                        {hasSTL && (
                          <button
                            onClick={() => {
                              if (order.file_url) {
